...
resolution_label = ttk.Label(root, text="Resolution (Advanced, leave at 512 for SD15. Choose 448 if limited to 4GB VRAM.):")
...
rank_label = ttk.Label(root, text="Network Rank, increases LoRA complexity, but also file size & VRAM consumption (see README for more info).")
...
train_text_encoder_checkbox = ttk.Checkbutton(root, text="Train Text Encoder (Optional, do NOT select with \"Pre Compute Text Embeddings\")", variable=train_text_encoder_var)
...
train_text_encoder_checkbox.grid(row=19, column=1, sticky='w', pady=5)
...
train_text_encoder_checkbox.grid(row=21, column=0, sticky='w', pady=5)
